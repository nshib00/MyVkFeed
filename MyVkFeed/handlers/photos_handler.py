...
def filter_photos_by_size(
    photos: list, photo_count: int, excluded_types: tuple = ("x", "y", "z", "w")
):
    ...
    for photo_dict in photos:
        for photo in photo_dict:
            if photo["type"] not in excluded_types:
                allitems.append([photo["url"], photo["type"]])
    ...
    if len(urls_without_params) == 0:
        ...
        for index, item in enumerate(allitems):
            if (
                index < photo_count
            ):  # Первые photo_count-1 фото - это фото с наиболее подходящим для отображения разрешением.
                if photo_count in range(4, 7):
                    PhotoSort.priorities = {
                        "m": 0,
                        "p": 1,
                        "o": 2,
                        "s": 3,
...
def get_filtered_post_photos(
    photo_list: list, photo_counter: list, post_number: int, final_photo_refs_list: list
):
