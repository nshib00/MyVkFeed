...
class MyVkFeedLogic:
    ...
    def __init__(self, user_id):
        ...
        self.all_posts_photo_counter = [0] * (self.posts_count + 1) # счётчик фото в каждом из постов. Индекс списка - номер поста.

    def load_posts_from_vk(self):
        try:
            self.groups_req = requests.get(f'https://api.vk.com/method/groups.get?access_token={config.VK_API_TOKEN}&user_id={self.user_id}&extended=1&v=5.131')
            self.feed_req = requests.get(f'https://api.vk.com/method/newsfeed.get?&access_token={config.VK_API_TOKEN}&filters=post&count={self.posts_count}&v=5.131')
            ...
            logger.info(f'Код запроса к группам: {self.groups_req.status_code}, код запроса к стене: {self.feed_req.status_code}.')
    ...
    def get_group_titles_and_ids(self):
        for item in self.groups_info:
            self.group_titles_and_ids.append(
                                                (item['title'], item['id'])
                                             )
    ...
    def get_posts_photo_refs(self):
        for index, post in enumerate(self.data):
            if 'attachments' in post:
                post_photos = [item for item in post['attachments'] if item['type'] == 'photo']
            ...
            if 'copy_history' in post:
                for item in post['copy_history']:
                    if 'attachments' in item:
                        post_photos = [attachment for attachment in item['attachments'] if attachment['type'] == 'photo']
                        photos_handler.get_filtered_post_photos(
                            photo_list=post_photos,
                            photo_counter=self.all_posts_photo_counter,
                            post_number=index,
    ...
    def load_data(self):
        if self.user_id is not None:
            try:
                ...
                logger.info(f'Загружено постов: {len(self.data)}, удалено рекламных постов: {self.ads}.')
            finally:
                self.ads = 0
